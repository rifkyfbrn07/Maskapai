...
<div class="max-w-xl mx-auto py-4">
    <div class="mb-8 pb-6 border-b border-slate-900">
        ...
        <h1 class="text-3xl font-extrabold text-white font-display">Schedule New Flight</h1>
        <p class="text-sm text-slate-400 mt-1">Configure flight numbers, pricing matrix, and aircraft scheduling</p>
    </div>

    <div class="glass-card rounded-2xl p-6 border border-slate-900 relative overflow-hidden">
        <form action="{{ route('flights.store') }}" method="POST" class="space-y-5">
            @csrf

            <!-- Flight Number -->
            <div>
                <label for="flight_number" class="block text-sm font-semibold text-slate-300 mb-2">Flight Number</label>
                <input type="text" name="flight_number" id="flight_number" value="{{ old('flight_number') }}" required autofocus
                ...
                <div class="relative">
                    <select name="airplane_id" id="airplane_id" required
                        ...
                        @foreach($airplanes as $airplane)
                            <option value="{{ $airplane->id }}" {{ old('airplane_id') == $airplane->id ? 'selected' : '' }}>{{ $airplane->airline->name }} - {{ $airplane->model }} (Cap: {{ $airplane->capacity }} seats)</option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                <!-- Departure Airport -->
                <div>
                    <label for="departure_airport_id" class="block text-sm font-semibold text-slate-300 mb-2">Origin</label>
                    <div class="relative">
                ...
                <!-- Arrival Airport -->
                <div>
                    <label for="arrival_airport_id" class="block text-sm font-semibold text-slate-300 mb-2">Destination</label>
                    <div class="relative">
