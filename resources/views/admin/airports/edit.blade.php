...
<div class="max-w-xl mx-auto py-4">
    <div class="mb-8 pb-6 border-b border-slate-900">
        ...
        <h1 class="text-3xl font-extrabold text-white font-display">Edit Airport Hub</h1>
        <p class="text-sm text-slate-400 mt-1">Modify records for airport {{ $airport->name }} ({{ $airport->iata_code }})</p>
    </div>

    <div class="glass-card rounded-2xl p-6 border border-slate-900 relative overflow-hidden">
        <form action="{{ route('airports.update', $airport->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $airport->name) }}" required autofocus
            ...
            <div>
                ...
                <input type="text" name="iata_code" id="iata_code" value="{{ old('iata_code', $airport->iata_code) }}" required maxlength="3"
            ...
            <div>
                ...
                <input type="text" name="city" id="city" value="{{ old('city', $airport->city) }}" required
                    class="block w-full px-4 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-slate-200 text-sm focus:outline-none focus:ring-1 focus:ring-cyan-500 transition"
                    placeholder="Jakarta">
                @error('city')
                    <p class="mt-2 text-xs text-red-400 font-medium">{{ $message }}</p>
            ...
            <div>
                ...
                <input type="text" name="country" id="country" value="{{ old('country', $airport->country) }}" required
                    class="block w-full px-4 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-slate-200 text-sm focus:outline-none focus:ring-1 focus:ring-cyan-500 transition">
                @error('country')
                    <p class="mt-2 text-xs text-red-400 font-medium">{{ $message }}</p>
                @enderror
